...
class Validator {
  validate(parsedData) {
    try {
      ...
      validated.confirmation_number = this.validateConfirmationNumber(
    ...
    ['departure', 'return'].forEach((key) => {
      if (dates[key]) {
        try {
          ...
          if (
            typeof dates[key] === 'string' &&
            dates[key].match(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/)
          ) {
            // Already in correct format, don't convert
            validatedDates[key] = dates[key];
          } else {
            ...
            if (isNaN(date.getTime())) {
              ...
              validatedDates[key] = date.toISOString().slice(0, 16).replace('T', ' ');
            }
          }
        } catch (error) {
          validatedDates[key] = null;
