...
// Add this debugging middleware at the TOP of src/routes/admin.js
router.use((req, res, next) => {
  console.log(`🎯 Admin Route Handler: ${req.method} ${req.path}`);
  console.log(`🎯 Full route: ${req.baseUrl}${req.path}`);
  next();
});

// TEST ROUTE - Add this around line 15, right after your debugging middleware
router.get('/test-prompts', (req, res) => {
  console.log('🧪 TEST PROMPTS ROUTE HIT!');
  res.json({ message: 'Test prompts route working!' });
});

// MINIMAL TEST - Add this right after your debugging middleware
router.get('/prompts/test', (req, res) => {
...
router.get('/prompts/categories', async (req, res) => {
  ...
  try {
    ...
    console.log('🎯 Database query successful:', categories);
    ...
    console.error('🚨 Error in /prompts/categories:', error);
    res.status(500).json({ error: 'Failed to fetch prompt categories', details: error.message });
  }
});

// GET /api/v1/admin/prompts/types - Get available prompt types
router.get('/prompts/types', async (req, res) => {
  console.log('🎯 INSIDE /prompts/types handler - START');
  try {
    console.log('🎯 About to query database for types...');
    ...
    console.log('🎯 Database query successful:', types);
    ...
    console.error('🚨 Error in /prompts/types:', error);
    res.status(500).json({ error: 'Failed to fetch prompt types', details: error.message });
  }
});
...
// =====================================================================
// PROMPTS MANAGEMENT - PARAMETERIZED ROUTES (MUST BE LAST)
// =====================================================================
...
// =====================================================================
// ENHANCED TEST CLASSIFICATION ENDPOINT
// =====================================================================
