// scripts/restoreOriginalPrompts.js - Restore the EXACT working prompts
const { PrismaClient } = require('@prisma/client');

const prisma = new PrismaClient();

async function restoreOriginalPrompts() {
  console.log('Restoring original working prompts...');

  // Delete the current broken prompts
  await prisma.promptTemplate.deleteMany({});
  await prisma.aIConfiguration.deleteMany({});

  console.log('Deleted old prompts');

  // 1. Classification prompt (this one was working)
...
Respond with only the classification word.`,
  ...
  // 2. Base parsing prompt - EXACT REPLICA of the original working one
  await prisma.promptTemplate.create({
    data: {
...
{{emailContent}}`,
  ...
  // 3. EXACT FLIGHT prompt from the original working code
  await prisma.promptTemplate.create({
    data: {
      ...
      version: 1,
      isActive: true,
      prompt: `You are an expert flight email parser. Extract ALL flight information from this confirmation email.
You MUST respond with valid JSON only. Do not include any explanatory text outside the JSON.
...
{{emailContent}}`,
  ...
  // 4. EXACT HOTEL prompt from the original working code
  await prisma.promptTemplate.create({
    data: {
...
IMPORTANT: check_in_time and check_out_time must be in 24-hour format!`,
  ...
  // 5. EXACT CAR RENTAL prompt from the original working code
  await prisma.promptTemplate.create({
    data: {
...
IMPORTANT: pickup_time and return_time must be in 24-hour format!`,
  ...
  // 6. AI Configuration (same as before)
  await prisma.aIConfiguration.create({
    data: {
      ...
      metadata: {
        ...
        description: 'Primary production configuration - restored original prompts',
      },
    },
  });

  console.log('✅ Restored original working prompts');
}

async function main() {
  try {
    await restoreOriginalPrompts();
  } catch (error) {
...
module.exports = { restoreOriginalPrompts };
